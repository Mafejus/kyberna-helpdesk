...
export class NotificationPreferencesService {
  ...
  async updatePreferences(
    ...
    for (const update of updates) {
      if (!allowedTypes.includes(update.type)) {
        throw new ForbiddenException(
          `Notification type ${update.type} is not allowed for role ${user.role}`,
        );
      }
    ...
    const currentSettings =
      (existing?.settings as Record<string, boolean>) || {};
  ...
  async shouldSend(user: User, type: NotificationType): Promise<boolean> {
    ...
    if (!allowedTypes || !allowedTypes.includes(type)) {
      console.warn(
        `Attempted to send notification ${type} to user ${user.id} (Role: ${user.role}) but it is not allowed for this role.`,
      );
      return false;
    }
