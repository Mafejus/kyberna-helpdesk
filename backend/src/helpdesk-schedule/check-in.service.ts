...
export class CheckInService {
  ...
  async startCheckIn(user: User, date: string, lesson: number) {
    ...
    const parsePragueTime = (dateStr: string, timeStr: string) => {
      ...
      const now = new Date();
      const pragueNowStr = new Intl.DateTimeFormat('en-CA', {
        // YYYY-MM-DD ...
      ...
      const nowPragueVal = new Date(
        Date.UTC(pYear, pMonth - 1, pDay, pHour, pMinute),
      ).getTime();
      const lessonStartVal = new Date(
        Date.UTC(lYear, lMonth - 1, lDay, lStartH, lStartM),
      ).getTime();
    ...
    console.log(
      `[CheckIn Debug] Now: ${new Date(nowPragueVal).toISOString()} (Val: ${nowPragueVal}), Start: ${times.start} (Val: ${lessonStartVal}), Allow: ${new Date(allowStartVal).toISOString()}`,
    );

    if (nowPragueVal < allowStartVal) {
      const diffM = Math.ceil((allowStartVal - nowPragueVal) / 60000);
      throw new BadRequestException(
        `Check-in možný nejdříve 15 minut před začátkem. (Za ${diffM} min)`,
      );
    }
    if (nowPragueVal > lessonEndVal) {
      throw new BadRequestException(
        `Služba již skončila. (Konec: ${times.end})`,
      );
    }
