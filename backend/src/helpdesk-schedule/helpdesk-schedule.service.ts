...
export class HelpdeskScheduleService {
  ...
  async getCurrentStatus() {
    ...
    const getPart = (type: Intl.DateTimeFormatPartTypes) => {
        const formatter = new Intl.DateTimeFormat('en-US', { timeZone, [type]: 'numeric', hour12: false });
    ...
    // Debug log
    console.log(`Time check (Prague): ${todayDate} ${hour}:${minute} (${currentMinutes}m), Day: ${weekday}`);
    ...
    if (!isWeekend) {
      ...
      if (!currentSlot) {
         ...
         const lessonIds = Object.keys(LESSON_TIMES).map(Number).sort((a, b) => a - b);
    ...
    if (!currentSlot && !nextSlot) {
        ...
        let d = new Date(now); // start from "now" object which is UTC but ...
  ...
  async unclaimSlot(user: User, date: string, lesson: number) {
    ...
    if (diffHours < 48) {
      throw new ForbiddenException('Slot lze zrušit nejpozději 48 hodin před začátkem.');
    }
