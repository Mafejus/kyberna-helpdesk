...
import { User, Role, AuditAction, AuditEntityType, NotificationType, SwapStatus } from '@prisma/client';
...
export class SwapService {
  ...
  async createOffer(user: User, date: string, lesson: number) {
    ...
    console.log(`[SwapService] Swap offer: User=${user.id} Date=${date} Lesson=${lesson} ShiftEnd=${shiftEnd.toISOString()}`);
    ...
    const filteredIds = await this.preferencesService.filterRecipients(otherStudents, NotificationType.SWAP_OFFER_CREATED);
  ...
  async acceptOffer(user: User, swapId: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
        ...
        if (swap.status !== SwapStatus.OPEN) throw new BadRequestException('Swap offer not available');
        if (swap.offeredByUserId === user.id) throw new BadRequestException('Cannot accept own offer');
        ...
        await tx.helpdeskShiftAssignee.delete({
            where: { shiftId_userId: { shiftId: swap.shiftId, userId: swap.offeredByUserId } }
        });
  ...
  async cancelOffer(user: User, swapId: string) {
      const swap = await this.prisma.helpdeskSwap.findUnique({ where: { id: swapId } });
      ...
      if (swap.status !== SwapStatus.OPEN) throw new BadRequestException('Can only cancel OPEN offers');
