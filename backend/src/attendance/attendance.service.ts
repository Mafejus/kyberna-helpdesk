...
export class AttendanceService {
  ...
  private calculateStatus(
    ...
    const lateThreshold = new Date(
      shiftStart.getTime() + ATTENDANCE_TOLERANCE.LATE_MINUTES * 60000,
    );
    const noShowThreshold = new Date(
      shiftStart.getTime() + ATTENDANCE_TOLERANCE.NO_SHOW_MINUTES * 60000,
    );
  ...
  async getAttendanceSummary(from: string, to: string) {
    ...
    const summary = students.map((student) => {
      ...
      for (const assignment of student.helpdeskShifts) {
        ...
        const { status, minutesWorked } = this.calculateStatus(
          shift.date,
          shift.lesson,
          checkIn,
          now,
  ...
  async getWeekCalendar(start: string) {
    ...
    for (let i = 0; i < 5; i++) {
      const dateStr = new Date(d.getTime() + i * 86400000)
        .toISOString()
        .split('T')[0];
      dates.push(dateStr);
    }
  ...
  async exportCsv(from: string, to: string) {
    ...
    // Generate CSV string
    const header =
      'Student,Email,Total Shifts,Worked Shifts,Late,No-Show,Missing Checkout,Total Minutes\n';
    const rows = data
      .map(
        (d) =>
